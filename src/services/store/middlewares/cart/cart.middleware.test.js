...
import cartMiddleware from "./index";
import React from "react";
import {cartInitialStore} from "../../reducers/cart/cart.initialStore";
...
import {
    ...
    SAVE_CART_START,
    FETCH_CART_END,
    FETCH_CART_OFFERS_START, SET_OFFERS
} from "../../actions/cart/cart.actions.types";
import axios from "axios";
import {fetchOffers} from "./fetchOffers"
import {setCart, saveCart} from "../../actions/cart/cart.actions";


const create = () => {
    const store = {
        getState: jest.fn(() => ({cart:cartInitialStore})),
        dispatch: jest.fn()
    };
    ...
    const invoke = action => cartMiddleware(store)(next)(action);

    return { store, next, invoke }
};

const createWithCart = () => {
    const store = {
        getState: jest.fn(() => ({cart:{...cartInitialStore, cart: cart}})),
        dispatch: jest.fn()
    };
...
describe('Cart middle suite test', () => {

    beforeEach(() => {
        localStorage.setItem("henriPottierBooksCart", JSON.stringify((cart)));
    });

    it('Get initial cart', () => {
        const { next, invoke, store } = create();
        const action = { type: FETCH_CART };
        invoke(action);
        expect(next).toHaveBeenCalledWith(action);
        expect(store.dispatch).toHaveBeenCalledWith({payload: cart, type: SET_CART});
    });

    it('Saves cart in localStorage on cart update', () => {
        const { next, invoke, store } = createWithCart();
        const action = { type: SET_CART, payload: cart };
        invoke(action);
        expect(next).toHaveBeenCalledWith(action);
        expect(store.dispatch).toHaveBeenCalledWith({type: SAVE_CART_END});
    });

    it('Updated cart amount on cart update', () => {
        const { next, invoke, store } = createWithCart();
        const action = { type: SET_CART, payload: cart };
        invoke(action);
        expect(next).toHaveBeenCalledWith(action);
        expect(store.dispatch).toHaveBeenCalledWith({type:  UPDATE_CART_AMOUNT, payload: book.price});
    });

    it('Fetches offers on cart update', async () => {
        ...
        const { next, invoke, store } = createWithCart();
        const action = { type: UPDATE_CART_AMOUNT, payload: book.price };
        invoke(action);
        expect(next).toHaveBeenCalledWith(action);
        await fetchOffers(store.dispatch, store.getState);
        expect(store.dispatch).toHaveBeenCalledWith({type:  SET_OFFERS, payload: offers});
    });
});
